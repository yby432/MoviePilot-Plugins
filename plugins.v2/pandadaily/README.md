...
- 支持多个派遣栏位全局规划：优先填满栏位，再比较属性匹配度和单位时间收益
- 事务所可选择 6、12、18 或 24 小时委托，到期 2 分钟后自动领取续派
- 每日工作、互动和收益与事务所完全独立，仍按每日任务周期执行
- 每日任务与事务所任务同时触发时，后到的任务会延迟 2 分钟自动补跑
...
- 重试间隔秒数，默认 `60`
- 自动派遣事务所，默认开启
- 事务所智能分配，默认开启；关闭后按固定派遣时长选择
- 事务所派遣时长，支持 `6`、`12`、`18`、`24` 小时，默认 `6`
